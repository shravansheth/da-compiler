...
;; Fundtion to write a phi instruction in string llvm
(define (write-phi [phi : Phi] [block : BasicBlock]) : String
  (define ops (string-join (map (lambda (val block)
                     (format "[~a, %label~a]"
                             (if (Register? val)
                                 (symbol->string (Register-reg val))
                                 val)
                             block))
                   (Phi-values phi) (Phi-blocks phi)) ", "))
  (format "~a = phi ~a ~a"
          (symbol->string (Register-reg (Phi-result-reg phi)))
          (Register-type (Phi-result-reg phi))
          ops))
  ...
  (define (write-instruction instr)
    (define instr-str
      (match instr
         ...
         (if (register? val)
             ...
             (begin
               (if (string=? "void" (cast val String)) 
                   ...
                   (if (or (string=? "true" (cast val String)) (string=? "false" (cast val String)))
                       (format "ret i1 ~a" val)
                       (if (string-contains? (cast val String) "null")
                           (format "ret ~a" val)
                           (format "ret i64 ~a" val))))))]
        [(CondBr cond #;(Register cond-sym cond-ty) true-label false-label)
         (if (string? cond)
             (format "br i1 ~a, label %label~a, label %label~a" cond true-label false-label)
             (format "br i1 ~a, label %label~a, label %label~a" (Register-reg (cast cond Register)) true-label false-label))
         ]
        [(Br lab)
        ...
        [(Phi reg values blocks)
         (write-phi instr block)]))
...
(define (main-write [file : String] [cfg : (Listof FuncBlock)])
